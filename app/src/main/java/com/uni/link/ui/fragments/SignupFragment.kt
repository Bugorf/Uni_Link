...
/**
 *  Fragment pour traiter inscription
 **/
...
class SignupFragment : BaseFragment() {
    ...
    private fun register() {
        ...
        if (password != confirmPw) {
            signupConfirmPassword.error = "MDP ne sont pas le meme"
            return
        }
        ...
        if (!imageSelected) {
            activity?.toast("Veuillez choisire une image de profil")
            return
        }

        // Check username
        if (!AppUtils.isValidUsername(name)) {
            signupUsername.error = "Nom d'utilisateur invalide"
            return
        }
